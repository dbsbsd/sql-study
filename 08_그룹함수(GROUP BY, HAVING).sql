...
-- 주의할 점
-- 그룹 함수는 일반 컬럼과 동시에 그냥 출력 할 수는 없습니다.
SELECT
    department_id,
    AVG(salary)
FROM employees; -- 에러
...
*/
SELECT
    TO_CHAR(hire_date, 'YY"년"') AS 입사년도,
    count(*) || '명' AS 사원수
FROM employees
GROUP BY TO_CHAR(hire_date, 'YY"년"')
ORDER BY 입사년도;
...
SELECT
    ...
    count(*) || '명' AS 사원수,
    TO_CHAR(TRUNC(AVG(salary + (salary * commission_pct)), 2), '$99,999.99') AS 평균,
    TO_CHAR(SUM(salary + (salary * commission_pct)), '$999,999') AS 합계
FROM employees
